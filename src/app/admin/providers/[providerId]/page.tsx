...
export default function AdminProviderDetailPage() {
  ...
  const reviews = [
    ...
    },
    {
      user: "Mike Wilson",
      rating: 4,
      comment: "Great gym, gets crowded during peak hours.",
      date: "2024-02-11",
    },
    {
      user: "Lisa Brown",
      rating: 5,
      comment: "Best gym in LA!",
      date: "2024-02-09",
    },
  ];
  ...
  const handleSuspendProvider = () => {
    requestConfirmation({
      ...
      description: `${provider.name}'s services will be temporarily unavailable.`,
  ...
  const handleRevokeVerification = () => {
    requestConfirmation({
      ...
      description: `${provider.name} will lose their verified badge immediately.`,
      confirmLabel: "Revoke Verification",
      onConfirm: async () => {
        await showAlert("Verification revoked successfully");
      },
    });
  };
  ...
  const handleDeleteProvider = async () => {
    const confirmation = await showPrompt({
      ...
      message: `Type "${provider.email}" to confirm permanent deletion:`,
      placeholder: provider.email,
      confirmLabel: "Delete",
    });
    if (confirmation === provider.email) {
      await showAlert("Provider deleted permanently");
      router.push("/admin/providers");
    } else if (confirmation) {
      await showAlert("Confirmation failed. Provider not deleted.");
    }
  };
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span
                    className={`px-3 py-1 text-sm font-semibold ${getTypeBadgeColor(provider.type)}`}
                  >
            ...
            <div className="space-y-6">
              ...
              {provider.verified && (
                <div className="bg-accent-yellow-50 border-2 border-accent-yellow-500 p-6">
                  <h3 className="text-lg font-bold text-accent-yellow-700 mb-4">
                    Verification
                  </h3>
                  <button
              ...
              <div className="bg-red-50 border-2 border-red-500 p-6">
                ...
                <p className="text-xs text-red-600 mt-2">
                  This will delete all provider data, plans, and member
                  subscriptions.
                </p>
              </div>
            </div>
