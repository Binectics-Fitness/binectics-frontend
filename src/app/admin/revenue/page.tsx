...
export default function AdminRevenuePage() {
  ...
  const handleProcessPayout = (provider: string, amount: string) => {
    requestConfirmation({
      ...
      description: `Process a payout of ${amount} to ${provider}.`,
      confirmLabel: "Process Payout",
      confirmVariant: "primary",
      onConfirm: async () => {
        await showAlert("Payout processed successfully");
      },
    });
  };
        ...
        <header className="bg-white border-b border-gray-200">
          <div className="px-4 py-4 sm:px-6 sm:py-5 md:px-8 md:py-6">
            ...
            <p className="mt-1 text-foreground/60">
              Track platform earnings and provider payouts
            </p>
          </div>
        </header>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {transactions.map((txn) => (
                        <tr key={txn.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-3 py-1 text-xs font-semibold ${getStatusBadgeColor(txn.status)}`}
                            >
                              {txn.status}
                            </span>
                          </td>
                  ...
                  {payoutQueue.map((payout, index) => (
                    ...
                    >
                      ...
                      {payout.status === "Ready" && (
                        <button
                          onClick={() =>
                            handleProcessPayout(payout.provider, payout.amount)
                          }
