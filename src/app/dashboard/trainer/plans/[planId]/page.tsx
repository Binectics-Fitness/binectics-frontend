  ...
  const handleDeletePlan = () => {
    ...
    requestConfirmation({
      ...
      onConfirm: async () => {
        ...
        if (response.success) {
          router.push("/dashboard/trainer/plans");
          return;
        }
      ...
      <div className="flex min-h-screen bg-background">
        ...
        <main className="md:ml-64 flex-1 p-6 md:p-8">
          <div className="max-w-4xl mx-auto">
            ...
            <Link href="/dashboard/trainer/plans" className="inline-flex mt-4 text-accent-yellow-600 hover:text-accent-yellow-700 font-medium">
              Back to plans
            </Link>
          </div>
        </main>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
            ...
            <div className="flex gap-3">
              <Link
                href={`/dashboard/trainer/plans/${plan._id}/edit`}
                className="px-4 py-2 border-2 border-accent-yellow-500 text-accent-yellow-700 font-semibold rounded-lg hover:bg-accent-yellow-50"
              >
                Edit Plan
              </Link>
              <button
              ...
              <ul className="space-y-2">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center gap-2 text-foreground">
                    <span className="inline-flex h-2 w-2 rounded-full bg-accent-yellow-500" />
                    {feature}
                  </li>
                ))}
              </ul>
