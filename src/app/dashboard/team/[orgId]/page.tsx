...
export default function OrgDetailPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      if (!orgRes.success) setError(orgRes.message ?? "Failed to load organization.");
    } catch {
  ...
  async function handleRemoveMember(memberId: string) {
    ...
    await teamsService.removeMember(orgId, memberId);
    setMembers((prev) => prev.filter((m) => m._id !== memberId));
  }

  async function handleDeactivateMember(memberId: string, current: MemberStatus) {
    const newStatus = current === MemberStatus.ACTIVE ? MemberStatus.INACTIVE : MemberStatus.ACTIVE;
    const res = await teamsService.updateMember(orgId, memberId, { status: newStatus });
    if (res.success && res.data) {
      setMembers((prev) =>
  ...
  async function handleCancelInvitation(invitationId: string) {
    ...
    await teamsService.cancelInvitation(orgId, invitationId);
    setInvitations((prev) =>
      prev.map((i) =>
        i._id === invitationId ? { ...i, status: InvitationStatus.CANCELLED } : i,
      ),
    );
  }
  ...
  async function handleDeleteRole(roleId: string) {
    if (!confirm("Delete this role? Members using it will keep their assignment until updated.")) return;
    ...
    return (
      <div className="flex min-h-screen bg-background">
        ...
        <main className="flex-1 p-8 ml-64">
          ...
          <Link href="/dashboard/team" className="mt-4 inline-block text-sm text-accent-blue-500 hover:underline">
            ← Back to Team
          </Link>
        </main>
      </div>
      ...
      <main className="flex-1 p-8 ml-64">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-foreground">{org.name}</h1>
              {org.description && (
                <p className="mt-0.5 text-sm text-foreground-secondary">{org.description}</p>
              )}
            </div>
          </div>
          <button
        ...
        <div className="mb-8 grid grid-cols-3 gap-4">
          <div className="rounded-xl border border-neutral-200 bg-white p-4">
            <p className="text-2xl font-black text-foreground">{members.filter((m) => m.status === MemberStatus.ACTIVE).length}</p>
            <p className="mt-0.5 text-xs text-foreground-secondary">Active Members</p>
          </div>
          <div className="rounded-xl border border-neutral-200 bg-white p-4">
            <p className="text-2xl font-black text-foreground">{roles.filter((r) => !r.is_default).length}</p>
            <p className="mt-0.5 text-xs text-foreground-secondary">Custom Roles</p>
          </div>
          <div className="rounded-xl border border-neutral-200 bg-white p-4">
            <p className="text-2xl font-black text-foreground">{invitations.filter((i) => i.status === InvitationStatus.PENDING).length}</p>
            <p className="mt-0.5 text-xs text-foreground-secondary">Pending Invitations</p>
          </div>
        </div>
        ...
        <div className="border-b border-neutral-200 mb-6">
          <nav className="flex gap-2 -mb-px">
            ...
            <button className={tabClass("invitations")} onClick={() => setActiveTab("invitations")}>
              Invitations ({invitations.filter((i) => i.status === InvitationStatus.PENDING).length})
            </button>
          </nav>
        </div>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white overflow-hidden">
            {members.length === 0 ? (
              <div className="py-16 text-center text-sm text-foreground-secondary">
                No members yet.{" "}
                <button onClick={() => setShowInviteModal(true)} className="text-accent-blue-500 hover:underline font-semibold">
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-neutral-100 bg-neutral-50">
                  <tr>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Member</th>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Role</th>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Status</th>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Joined</th>
                ...
                <tbody className="divide-y divide-neutral-50">
                  {members.map((member) => (
                    <tr key={member._id} className="hover:bg-neutral-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-foreground-secondary">{member.user?.email ?? "—"}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${MEMBER_STATUS_STYLES[member.status]}`}>
                          {member.status}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => handleDeactivateMember(member._id, member.status)}
                          ...
                          >
                            {member.status === MemberStatus.ACTIVE ? "Deactivate" : "Activate"}
                          </button>
                          <button
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-foreground-secondary">
                Default roles are shared across all organizations. Custom roles are specific to this organization.
              </p>
              <button
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs font-mono text-foreground-secondary">{role.code}</p>
                    </div>
                    {!role.is_default && (
                      <button
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white overflow-hidden">
            {invitations.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-neutral-100 bg-neutral-50">
                  <tr>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Email</th>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Role</th>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Status</th>
                    <th className="px-6 py-3 text-left font-semibold text-foreground-secondary">Expires</th>
                    <th className="px-6 py-3" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-50">
                  {invitations.map((inv) => (
                    <tr key={inv._id} className="hover:bg-neutral-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${INVITATION_STATUS_STYLES[inv.status]}`}>
                          {inv.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-foreground-secondary">
            ...
            <form onSubmit={handleInvite} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setInviteForm((f) => ({ ...f, team_role_id: e.target.value }))}
      ...
      {showRoleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-neutral-100 p-6">
              <h2 className="text-xl font-bold text-foreground">Create Custom Role</h2>
              <button
