...
export default function DietitianTodayClient() {
  ...
  const kpis = [
    ...
    { label: "Consults · today", value: String(consultsToday), sub: consultsToday === 0 ? "Nothing scheduled" : "Confirmed + pending" },
    ...
    <DietitianDashboardShell activeItem="Today" crumb="Today" actions={<NewPlanButton />}>
      ...
      <div>
        ...
        <div className="text-[13.5px] mt-1.5" style={{ color: "var(--fg-3)" }}>
          {upcoming.length > 0 ? `${upcoming.length} upcoming · ${activeClients.length} active clients` : "Your nutrition practice overview"}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-3 items-start">
            ...
            <div className="rounded-(--r-3) overflow-hidden" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
              <div className="flex items-center justify-between px-4.5 py-3.5" style={{ borderBottom: "1px solid var(--border)" }}>
                <div>
                  <h3 className="text-[14px] font-medium" style={{ letterSpacing: "-0.005em", color: "var(--ink)" }}>Upcoming consults</h3>
                  ...
                  return (
                    <div key={b.id} className="grid gap-3 px-4.5 py-3 items-center" style={{ gridTemplateColumns: "auto 1fr auto", borderBottom: i < arr.length - 1 ? "1px solid var(--border)" : "none" }}>
                      ...
                      <div className="min-w-0">
                        <div className="text-[13.5px] font-medium" style={{ color: "var(--ink)" }}>{durationMins(b)} min consult</div>
