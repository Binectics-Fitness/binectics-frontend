          ...
          {error ? (
            ...
            <div className="flex flex-col gap-2">
              {instances.map((i) => (
                <Link
                  key={i._id}
                  href={`/dashboard/dietitian/programs/instances/${i._id}`}
...
const STATUS_BADGE: Record<ProgramTemplate["status"], { bg: string; color: string; label: string }> = {
  ...
  published: { bg: "var(--dietitian-soft)", color: "var(--dietitian)", label: "Published" },
      ...
      <div className="flex items-center justify-between gap-2 px-5.5 py-3 flex-wrap" style={{ borderTop: "1px solid var(--border)" }}>
        <div className="flex gap-2 flex-wrap">
          ...
          {isPublished && template.status !== "archived" && (
            <button type="button" onClick={onAssign} className="font-mono text-[10.5px] uppercase tracking-[0.04em] px-2.5 py-1.25 rounded-(--r-1)" style={{ border: "1px solid var(--border)", color: "var(--dietitian)", background: "transparent" }}>
              Assign
            </button>
          )}
...
export default function ProgramsClient({ initialCreateOpen = false }: { initialCreateOpen?: boolean }) {
  ...
  useEffect(() => {
    if (initialCreateOpen) router.replace("/dashboard/dietitian/programs", { scroll: false });
  ...
  return (
    <DietitianDashboardShell activeItem="Programs" crumb="Programs">
      ...
      {clientsTemplate && (
        <AssignmentsModal template={clientsTemplate} onClose={() => setClientsTemplate(null)} />
      )}
