...
| Async states consistency | Technical Debt | n/a | in-progress | Shared AsyncSpinner/EmptySlate primitives exist and are now used by all converted dashboards. Sweeping remaining data routes onto them: done so far — trainer/clients, dietitian/foods, gym-owner/facility. Remaining hand-rolled: dashboard/bookings, trainer/sessions, trainer/plans, dietitian/plans, dietitian/feedback, marketplace, admin/listings, admin/users | high | S-M | FE | Continue converting remaining data routes to AsyncSpinner/EmptySlate | in-progress |
