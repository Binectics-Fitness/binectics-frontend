...
| Recurring booking | Frontend Exists -> Backend Missing | partial/unclear | shipped | FE flow is built and reachable: /booking/recurring (linked from the one-time /booking flow via a "Need a fixed weekly slot?" prompt). Lets the user pick cadence (weekly/biweekly/monthly), weekday, time, and session count, previews the schedule + total, and creates each occurrence via repeated createBooking() (client-side recurrence) with a success/failure summary. Polished spinners to AsyncSpinner. LIMITATIONS (BE follow-up): no backend recurrence contract — it fires N individual bookings rather than a managed series, and times are from a fixed list (not provider availability), so unavailable occurrences fail individually and are reported | medium | M-H | FE+BE | BE: define a recurrence/series contract (RRULE-lite) + availability-aware slot selection | shipped |
| Dashboard shell duplication | Alignment/Refactor Opportunity | n/a | shipped | Extracted ProviderShell (components/ds): the gym/trainer/dietitian shells were three near-identical files (same sidebar body, nav rendering, user footer, role guard) differing only in nav config, accent, identity source (org vs user), and role. Each is now a thin config wrapper (49-63 lines) over the shared scaffold; the duplicated logic lives in one place. Admin shell intentionally left bespoke (distinct dark theme + own mobile nav) | medium | M | FE | Monitor; fold Admin in only if it converges on the same layout | shipped |
...
| Async states consistency | Technical Debt | n/a | shipped | Shared AsyncSpinner/EmptySlate primitives used across the data routes. Converted: trainer/clients, dietitian/foods, gym-owner/facility, dietitian/feedback, marketplace, marketplace/[listingId], admin/listings, dashboard/bookings (loading). Intentionally left rich states that EmptySlate can't express: trainer/plans & dietitian/plans (skeleton loaders + empty WITH a create-plan CTA), bookings EmptyState (per-tab + CTA). Acceptable inline patterns left as-is: header-count "Loading…" subtitles (trainer/sessions, dietitian/feedback header) and KPI "—" placeholders (admin/users, admin/listings) | high | S-M | FE | Monitor; use primitives for new routes | shipped |
...
| API contract typing normalization | Technical Debt | complete | in-progress | Foundations laid: (1) regenerated the OpenAPI types (src/lib/api/generated/schema.ts) so they match the API's current contract — api:check now passes; (2) typed the HTTP-client response boundary — removed all 4 `any`s in client.ts (request bodies any→unknown; responses parsed into a typed RawResponseBody then narrowed to ApiResponse<T>), behaviour-preserving. The generated schema is now current and ready to be consumed. Pattern established: src/lib/api/generated/types.ts re-exports contract types under friendly aliases, and consultations.ts now sources CreateBookingRequest = CreateBookingDto (zero drift) as the template. Remaining: migrate the rest of the hand-written service/types to reference the generated schema (large, incremental) | medium | M | FE+BE | Incrementally adopt generated schema types in the remaining services following the consultations template | in-progress |
