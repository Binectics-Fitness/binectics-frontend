...
| API contract typing normalization | Technical Debt | complete | in-progress | Foundations laid: (1) regenerated the OpenAPI types (src/lib/api/generated/schema.ts) so they match the API's current contract — api:check now passes; (2) typed the HTTP-client response boundary — removed all 4 `any`s in client.ts (request bodies any→unknown; responses parsed into a typed RawResponseBody then narrowed to ApiResponse<T>), behaviour-preserving. The generated schema is now current and ready to be consumed. Remaining: migrate hand-written service/types to reference the generated schema (large, incremental) | medium | M | FE+BE | Incrementally adopt generated schema types in services; add a typed-adapter pattern | in-progress |
